...
sentence = "python and java are object oriented programming languages"
# Matches both "python" and "java" Grouping
result = findall(r'(java|python)', sentence)

sentence = "the meeting is between 12:59 am and 09:30 pm"
# match 9:00 am and 12:30 pm
# 01:30 am, 09:45 am, 11:59 am 12:30 pm, 1:30 pm

result = findall(r'(?:0[1-9]|1[0-2]):[0-5][0-9]\s[ap]m', sentence)

result = findall(r"\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12][0-9]|3[01])", "9999-01-31")

# Inbox, Inbox(1), Inbox(2)....
# result = findall(r'(Inbox|Inbox\(\d+\))', "Inbox(10)")

result = findall(r"Inbox\(?\d*\)?", "Inbox(10)")

result = finditer(r"hello", "hi there hello world welcome hello hi hello world")

occurances = [ ]
# occurances = [ (item.start(), item.end()) for item in result ]

for item in result:
    occurances.append((item.start(), item.end()))

# Replace white spaces with new line character
sentence = "hello world welcome to python"
...
sentence = "hello world welcome to python"
s = sub(r"[aeiou]", "*", sentence)

# Replace all occurances of digits with "*"
sentence = "h34ello w3333orld65645 656wel45645come to p4645ython"
s = sub(r"\d", "*", sentence)

# Replace all occuraces of special chars with "*"
sentence = "h34ello!@ w3333orld65645 656we%l45645com!@@#e to p4645ython"

result = sub(r"[^a-zA-Z0-9_\s]", "S", sentence)
result = sub(r"[^\w\s]", "S", sentence)
# result = sub(r"[\W]", "S", sentence)

# Replace "and" with "&"
sentence = "java and python are programming languages"

result = sub(r"and", "&", sentence)

with open(r"./data_files/sample.log", "r") as f:
    for line in f:
        result = findall(r"UDP$", line)
        if result:
            print(line)


sentence = "hello welcome to the the python programming and and the program"

# sentence = "hello world welcome to python world"

# result = finditer(r"(the)\s\1", sentence)

result = finditer(r"([a-zA-Z]+)\s\1", sentence)

sentence = "hello hurry programming"

result = finditer(r"([a-zA-Z])\1", sentence)

# Repeated number pattern
sentence = "hello 123345 welcome to 661 9889911 and 4545 4545"
result = finditer(r"(\d)\1", sentence)